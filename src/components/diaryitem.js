...
const DiaryItemBase= (props) => {
  ...
  const [categorySelected, setCategorySelected] = useState('');
   ...
   useEffect(() => {
    ...
    props.firebase.blogs().once('value').then(snapshot => {
      ...
      const articles = Object.keys(snapshotValue).map((key) => ({ ...snapshotValue[key], key }));
  ...
  return (
    <div>
      <Dropdown placeholder='Select categories' selection options={category} value={categorySelected} onChange={handleChange} />
      {
        article.slice(0).reverse().map((value)=>{
        if((searchResult == null || (value.title || '').includes(searchResult)) && (!categorySelected || value.category === categorySelected)) {
